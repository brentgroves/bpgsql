...
-- use floor for date only comparison
...
select @sql = N'SELECT * into ' + quotename(@table_name) + N' from hourlyoeevalues  WHERE Date_time_stamp BETWEEN @fromDate AND @toDate'
...
insert into #results (primary_key,start_week,end_week,part_number,workcenter_code,planned_production_count,actual_production_count,actual_vrs_planned_percent,scrap_count,downtime_minutes)
(
select
pk.primary_key,
CONVERT(VARCHAR, start_week ,101),
CONVERT(VARCHAR, end_week ,101),
