...
--//////////////////////////////////////////////////////////////
create table #sales_shipper_container
(
...
insert into #sales_shipper_container (shipper_line_key,serial_no,release_key,quantity)
(
select 
--sh.shipper_no,
--ss.shipper_status,
...
--select count(*) #sales_shipper_container from #sales_shipper_container  --54
--select * from #sales_shipper_container  -

--where loaded_date is null  --0

--/////////////////////////////////////////////////////////////
-- Group the shipper containers by release key to get the
...
--//////////////////////////////////////////////////////////////
create table #sales_shipper_container_group_release
(
release_key int,
qty_loaded decimal (18,3)
);

insert into #sales_shipper_container_group_release (release_key,qty_loaded)
(
select 
release_key,
sum(quantity) qty_loaded
from #sales_shipper_container
group by release_key
) 

--select count(*) #sales_shipper_container_group_release from #sales_shipper_container_group_release  --4

--select * from #sales_shipper_container_group_release

-- //////////////////////////////////////////////////////////////
-- Active sales release shippers.
-- We group shipper_container by release_key to get 'Loaded' quantity.
-- We could also pull the quantity directly from the shipper line more easily.
-- But in case there is a descrepency between the calculated 'Loaded' quantity
-- and the shipper_line quantity we can see how the shipper line quantity was derived.
-- //////////////////////////////////////////////////////////////
create table #sales_shipper_line
(
release_key int,
shipper_no varchar(50),
shipper_status varchar(50),
quantity int,  -- This sales release sum of these should be the same as the calculated 'Loaded' value from shipper_container.
...
insert into #sales_shipper_line (release_key,shipper_no,shipper_status,quantity,shipped_quantity,ship_date,active)
(
...
sh.shipper_no,
ss.shipper_status,
sl.quantity, --Can be used to calculate total release_key loaded values. Although we use an alternate method described below.
sr.quantity_shipped,
sh.ship_date,
ss.active
...
create table #sales_release_shippers
(
release_key int,
shippers 	varchar(MAX)
);

insert into #sales_release_shippers (release_key,shippers)
(
  select
  release_key,
  case
  when shippers is null then ''
  else shippers
  end shippers
  from
  (
    select distinct sl.release_key,sl.shipped_quantity,
    --STUFF((Select ','+ slx.shipper_no
    case
    when sl.shipped_quantity > 0 then
    (
    select top(1)
    slx.shipper_no
    from #sales_shipper_line slx
    ...
    from #sales_shipper_line sl  --11531
  )s1
)


--select count(*) #sales_release_shippers from #sales_release_shippers --5

--select * from #sales_release_shippers 
...
1	95112262	BM07935	936  651AB
2	95112263	BM07935	936  652AB
3	94177353	BM07936	126
4	95111237	BM07933	288
Row Count: 4
...
from #sales_shipper_container sc
...
left outer join part_v_part p -- 1 to 1
on pl.part_key=p.part_key 
group by sl.release_key,p.part_no
...
insert into #sales_release_no_rel_bal (release_key,part_key,customer,po,release,ship_to,part_no,cust_part,part_name,ship_date,shippers,due_date,rel_qty,qty_loaded, shipped,release_status,release_type)
(
  select 
  --top 10
  sr.release_key,
