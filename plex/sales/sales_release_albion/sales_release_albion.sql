-- Called by Customer_Release_Albion which Mimics Plex sales release screen
-- ToDo:  
...
-- select count(*) #sales_release_part from #sales_release_part --17
...
--	93	Pending	1  --??
 ...
 -- select count(*) #sales_active_shipper_container from #sales_active_shipper_container  --36
...
quantity decimal,
loaded_date smalldatetime,
ship_date smalldatetime,
active smallint

);

insert into #sales_all_shipper_container (release_key,serial_no,shipper_no,shipper_status,quantity,loaded_date,ship_date,active)
(
...
-- select count(*) #sales_all_shipper_container from #sales_all_shipper_container  -- 125
-- select * from #sales_all_shipper_container  
...
create table #sales_release_shipper
(
release_key int,
-- Why are we calling this field shipers since it never has
-- more than one shipper_no?
...
last_shipper varchar(50),
shipper_containers varchar(max)
);

/*
Summary of all shipper containers for a release_key.
*/
insert into #sales_release_shipper (release_key,last_shipped_shipper,last_active_shipper,last_shipper,shipper_containers)
(
  select
  release_key,last_shipped_shipper,last_active_shipper,last_shipper,shippers
  from
  (
    ...
    (
      select top(1)
      scx.shipper_no
      from #sales_all_shipper_container scx
      where scx.release_key=sc.release_key
      and scx.shipper_status = 'Shipped'
      order by scx.loaded_date desc
    ...
    (
      select top(1)
      case 
      when scx.active = 1 then scx.shipper_no + '(L)'
      else scx.shipper_no
      end shipper_no
      from #sales_all_shipper_container scx
      where scx.release_key=sc.release_key
    ...
    (
      SELECT SUBSTRING(
      (
         -- SELECT ',' + scx.shipper_no
          SELECT ', ' + scx.serial_no + '/' + CONVERT(VARCHAR(10), scx.loaded_date, 110) + '/' +scx.shipper_status+ '/qty:' + cast(scx.quantity as varchar(10))
          -- SELECT ', ' + scx.shipper_no + '/' + CONVERT(VARCHAR(10), scx.ship_date, 110) + '/qty:' + cast(scx.quantity as varchar(10))
          FROM #sales_all_shipper_container scx
          WHERE scx.release_key = sc.release_key
          order by scx.loaded_date desc
          FOR XML PATH('')), 3, 200000)    
    ) as shippers
    from #sales_all_shipper_container sc  --11531
    group by sc.release_key
    
  )s1
)

/*
select * from sales_v_shipper_container where serial_no = 'AB754921'
 	PCN	   Shipper_Line_Key	Serial_No	Release_Key	Quantity	Loaded_Date	          Shipper_Container_Key
1	300758	28509781	      AB754921	102554463	  48.000	  1/11/2021 12:27:52 PM	240467271
*/
-- select count(*) #sales_release_shipper from  #sales_release_shipper --70

-- select * from  #sales_release_shipper 
...
*/
--///////////////////////////////////////////////////////////
-- Add all the fields from various tables.
-- The only think still needed is the calculation
-- of the release balance.
--/////////////////////////////////////////////////////////
...
(
  ...
  case 
  when sh.last_shipper is null then ''
  else sh.last_shipper
  end as last_shipper,
  case 
  when sh.shipper_containers is null then ''
  else sh.shipper_containers
  end as shipper_containers,
  sr.due_date,
  sr.quantity rel_qty, 
  case 
  ...
  left outer join #sales_release_shipper sh  --1 to 1 
...
-- select count(*) #sales_release_no_rel_bal from #sales_release_no_rel_bal  --17
-- select *  from #sales_release_no_rel_bal  --554

--select count(*) count_scheduled from #sales_release_no_rel_bal  
--where release_status = 'Scheduled'  --4

/*
-- This is the final set where we calculate the release balance.
*/
