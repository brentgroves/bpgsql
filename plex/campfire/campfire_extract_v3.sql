...
LEFT OUTER JOIN Accounting_v_AR_Invoice_Dist_e AS ID
...
LEFT OUTER JOIN Common_v_Currency AS C
 ...
 -- Added this
LEFT OUTER JOIN Sales_v_Shipper_Line_e AS SSL
  ON SSL.PCN = ID.Plexus_Customer_No
  AND SSL.Shipper_Line_Key = ID.Shipper_Line_Key
...
GROUP BY
  ...
  AI.Invoice_Date,
  LT.Timezone_Offset,
  ID.Shipper_Line_Key,  -- added this
  SSL.Quantity,
  AI.Exchange_Rate,
  C.Currency_Key,
  AI.Currency_Code;
...
CTE_Month_Zero_Data AS
(
SELECT
    CSRBM.PCN,
    CSRBM.Part_Key,
    CSRBM.Month_Sequence,
    CSRBM.Pieces_Quantity,
    CSRBM.Revenue,
    CSRBM.Local_Material_Cost,
    CSRBM.Foreign_Material_Cost,
    CSRBM.Local_Direct_Labor_Cost,
    CSRBM.Foreign_Direct_Labor_Cost,
    CSRBM.Local_Variable_Overhead_Cost,
    CSRBM.Foreign_Variable_Overhead_Cost,
    CSRBM.Local_Fixed_Overhead_Cost,
    CSRBM.Foreign_Fixed_Overhead_Cost
FROM CTE_Summarized_Releases_By_Month AS CSRBM 
...
--    CDLI.Ship_To,
    SI.Base_Currency_Key,
    C.Currency_HTML,
    (SELECT TOP 1 AP.Period FROM Accounting_v_Period_e AS AP WHERE AP.Plexus_Customer_No = P.Plexus_Customer_No AND  AP.Begin_Date <= @Start_Date AND AP.End_Date >= @Start_Date) AS Period,
    ...
    ISNULL(CMZD.Local_Material_Cost,0) AS Local_Material_Cost,
    ISNULL(CMZD.Foreign_Material_Cost,0) AS Foreign_Material_Cost,
    ISNULL(CMZD.Local_Direct_Labor_Cost,0) AS Local_Direct_Labor_Cost,
    ISNULL(CMZD.Foreign_Direct_Labor_Cost,0) AS Foreign_Direct_Labor_Cost,
    ISNULL(CMZD.Local_Variable_Overhead_Cost,0) AS Local_Variable_Overhead_Cost,
    ISNULL(CMZD.Foreign_Variable_Overhead_Cost,0) AS Foreign_Variable_Overhead_Cost,
    ISNULL(CMZD.Local_Fixed_Overhead_Cost,0) AS Local_Fixed_Overhead_Cost,
    ISNULL(CMZD.Foreign_Fixed_Overhead_Cost,0) AS Foreign_Fixed_Overhead_Cost,
    ISNULL((SELECT CSRBM.Pieces_Quantity FROM CTE_Summarized_Releases_By_Month AS CSRBM WHERE CSRBM.PCN = P.Plexus_Customer_No AND CSRBM.Month_Sequence = 0 AND CSRBM.Part_Key = P.Part_Key ),0) AS CurrBacklogUnits, --AND CSRBM.Ship_To = CDLI.Ship_To
--    CASE
--      WHEN CSRBM.Month_Sequence = 0 THEN CSRBM.Pieces_Quantity
--      ELSE 0
--    END AS CurrBackLogUnits,
    ISNULL((SELECT CSRBM.Pieces_Quantity FROM CTE_Summarized_Releases_By_Month AS CSRBM WHERE CSRBM.PCN = P.Plexus_Customer_No AND CSRBM.Month_Sequence = 1 AND CSRBM.Part_Key = P.Part_Key ),0) AS BacklogUnits1, --AND CSRBM.Ship_To = CDLI.Ship_To
--    CASE
--     WHEN CSRBM.Month_Sequence = 1 THEN CSRBM.Pieces_Quantity
--      ELSE 0
--    END AS BackLogUnits1,
    ISNULL((SELECT CSRBM.Pieces_Quantity FROM CTE_Summarized_Releases_By_Month AS CSRBM WHERE CSRBM.PCN = P.Plexus_Customer_No AND CSRBM.Month_Sequence = 2 AND CSRBM.Part_Key = P.Part_Key ),0) AS BacklogUnits2, --AND CSRBM.Ship_To = CDLI.Ship_To
--    CASE
--      WHEN CSRBM.Month_Sequence = 2 THEN CSRBM.Pieces_Quantity
--      ELSE 0
--    END AS BackLogUnits2,
    ISNULL((SELECT CSRBM.Pieces_Quantity FROM CTE_Summarized_Releases_By_Month AS CSRBM WHERE CSRBM.PCN = P.Plexus_Customer_No AND CSRBM.Month_Sequence = 3 AND CSRBM.Part_Key = P.Part_Key ),0) AS BacklogUnits3, --AND CSRBM.Ship_To = CDLI.Ship_To
--    CASE
--      WHEN CSRBM.Month_Sequence = 3 THEN CSRBM.Pieces_Quantity
--      ELSE 0
--    END AS BackLogUnits3,
    ISNULL((SELECT CSRBM.Pieces_Quantity FROM CTE_Summarized_Releases_By_Month AS CSRBM WHERE CSRBM.PCN = P.Plexus_Customer_No AND CSRBM.Month_Sequence = 4 AND CSRBM.Part_Key = P.Part_Key ),0) AS BacklogUnits4, --AND CSRBM.Ship_To = CDLI.Ship_To
--    CASE
--      WHEN CSRBM.Month_Sequence = 4 THEN CSRBM.Pieces_Quantity
--      ELSE 0
--    END AS BackLogUnits4,
    ISNULL((SELECT CSRBM.Pieces_Quantity FROM CTE_Summarized_Releases_By_Month AS CSRBM WHERE CSRBM.PCN = P.Plexus_Customer_No AND CSRBM.Month_Sequence = 5 AND CSRBM.Part_Key = P.Part_Key ),0) AS BacklogUnits5, --AND CSRBM.Ship_To = CDLI.Ship_To
--    CASE
--      WHEN CSRBM.Month_Sequence = 5 THEN CSRBM.Pieces_Quantity
--      ELSE 0
--    END AS BackLogUnits5,
    ISNULL((SELECT CSRBM.Pieces_Quantity FROM CTE_Summarized_Releases_By_Month AS CSRBM WHERE CSRBM.PCN = P.Plexus_Customer_No AND CSRBM.Month_Sequence = 6 AND CSRBM.Part_Key = P.Part_Key ),0) AS BacklogUnits6 --AND CSRBM.Ship_To = CDLI.Ship_To
--    CASE
...
--    AND P.Part_Key = CDLI.Part_Key
  LEFT OUTER JOIN CTE_Month_Zero_Data AS CMZD
    ON CMZD.PCN = P.Plexus_Customer_No
    AND CMZD.Part_Key = P.Part_Key
  LEFT OUTER JOIN CTE_Summarized_Invoices AS SI
    ON SI.Plexus_Customer_No = P.Plexus_Customer_No
...
--    AND SI.Ship_To_Address = CDLI.Ship_To
  ...
  LEFT OUTER JOIN CTE_Summarized_Releases_By_Month AS CSRBM
    ON CSRBM.PCN = P.Plexus_Customer_No
    AND CSRBM.Part_Key = P.Part_Key
----    AND CSRBM.Ship_To = CDLI.Ship_To
  LEFT OUTER JOIN Common_v_Currency AS C
    ON C.Currency_Key = SI.Base_Currency_Key
--  WHERE CSRBM.Month_Sequence = 0
--  WHERE (CMZD.Part_Key IS NOT NULL OR SI.Part_Key IS NOT NULL OR CSRBM.Part_Key IS NOT NULL)
--  GROUP BY 
--    CDLI.PCN,
--    CDLI.Part_Key,
...
--SELECT * FROM #Invoices AS I --WHERE I.Part_Key = 3081441;
--SELECT COUNT(DISTINCT I.Part_Key) FROM #Invoices AS I;
--SELECT * FROM #Summarized_Invoices AS SI WHERE SI.Part_Key = 3081441;
--SELECT * FROM #Releases AS R WHERE R.Part_Key = 3081441;
--SELECT * FROM #Releases_with_Cost AS RWC WHERE RWC.Part_Key = 3081441;
--SELECT * FROM #Summarized_Releases_With_Cost AS SRWC WHERE SRWC.Part_Key = 3081441;
--SELECT * FROM CTE_Releases_By_Month AS CRBM  WHERE CRBM.Part_Key = 3081441 ORDER BY CRBM.PCN, CRBM.Part_Key, CRBM.Ship_To;
--SELECT * FROM CTE_Summarized_Releases_By_Month AS SRBM WHERE SRBM.Part_Key = 3081441;
--SELECT * FROM CTE_Summarized_Invoices AS SI ORDER BY SI.Plexus_Customer_No, SI.Part_Key, SI.Ship_To_Address;
--SELECT * FROM CTE_Distinct_Line_Items AS CDLI WHERE CDLI.Part_Key = 3081441;
--SELECT * FROM CTE_Final_Summary AS CFS WHERE CFS.Part_Key = 3081441;
...
--  CFS.Currency_HTML AS Currency,
  ISNULL(CFS.Inv_Currency_Code, 'USD') AS Currency,
  CFS.Period,
  ISNULL(CFS.Actual_Units,0) AS 'Actual (Units)',
  ISNULL(CFS.Foreign_Price_Plus_Tax,0) AS Actual_Local_Rev,
  ISNULL(CFS.Local_Price_Plus_Tax,0) AS Actual_USD_Rev,
  ISNULL(CFS.Local_Material_Cost,0) AS Actual_Local_Material_Cost,
  ISNULL(CFS.Local_Direct_Labor_Cost,0) AS Actual_Local_Direct_Labor_Cost,
  ISNULL(CFS.Local_Variable_Overhead_Cost,0) AS Actual_Variable_Local_Overhead_Cost,
  ISNULL(CFS.Local_Fixed_Overhead_Cost,0) AS Actuals_Local_Fixed_Cost,
  ISNULL(CFS.Foreign_Material_Cost,0) AS Actual_USD_Material_Cost,
  ISNULL(CFS.Foreign_Direct_Labor_Cost,0) AS Actual_USD_Direct_Labor_Cost,
  ISNULL(CFS.Foreign_Variable_Overhead_Cost,0) AS Actual_Variable_USD_Overhead_Cost,
  ISNULL(CFS.Foreign_Fixed_Overhead_Cost,0) AS Actuals_USD_Fixed_Cost,
...
WHERE (CFS.Actual_Units IS NOT NULL
        OR CFS.CurrBacklogUnits > 0 
        OR CFS.BacklogUnits1 > 0
        OR CFS.BacklogUnits2 > 0 
        OR CFS.BacklogUnits3 > 0
        OR CFS.BacklogUnits4 > 0 
        OR CFS.BacklogUnits5 > 0 
        OR CFS.BacklogUnits6 > 0)
and CFS.Part_Number = '10037203' -- DEBUG        
ORDER BY CFS.Part_Number
--*/
