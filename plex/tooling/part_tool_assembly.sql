...
-- select count(*) from part_v_tool where part_key is null  -- 681
-- select top 10 drawing_no,revision,* from part_v_tool where revision <> '' 
-- select count(*) cnt from part_v_tool_assembly a  -- 19
-- select * from part_v_tool_assembly a  -- 109
-- select count(*) cnt from part_v_tool_bom  --
...
/*
create table part_tool_assembly
(
id int,
pcn int,
part_key int,
...
/*
    ...
    on o.operation_key=ap.operation_key
    */
/*
Assembly No=T02
...
select * from accounting_v_accounting_job where name like '%10103344%'
...
-- (
    select 
    --top 100
    row_number() over(order by p.part_no,po.operation_no,a.assembly_no) id,
...
--    '' Location, -- Don't know
 ...
 --   a.update_date
    ...
    -- select count(*) cnt 
    from part_v_tool_assembly_e a  -- 100
    inner join part_v_tool_assembly_type_e at --100
    on a.plexus_customer_no=at.pcn
    and a.tool_assembly_type_key=at.tool_assembly_type_key  -- 1 to 1
    inner join part_v_tool_assembly_part_e ap  -- 100
    on a.plexus_customer_no=ap.pcn
    and a.assembly_key=ap.assembly_key  -- 1 to many
    inner join part_v_part_e p  -- 100
    on ap.pcn=p.plexus_customer_no
    and ap.part_key=p.part_key -- 1 to 1
    inner join part_v_operation_e o  -- 100
    on ap.pcn=o.plexus_customer_no
    and ap.operation_key=o.operation_key  -- 1 to 1
    inner join part_v_part_operation_e po  -- 100
    on ap.pcn=po.plexus_customer_no
    and ap.part_key=po.part_key
    and ap.operation_key=po.operation_key
    inner join part_v_tool_assembly_status_e s -- 833
    on a.plexus_customer_no = s.pcn 
    and a.tool_assembly_status_key=s.tool_assembly_status_key  -- 1 to 1
    where a.plexus_customer_no = @PCN

  --  where p.part_no = 'LC5C 5K651 CE'  --LC5C 5K651 CE
    -- inner join part_v_tool_inventory i 
  -- and a.add_by = 11728751  -- Me = 367. Not me 25 rows
      --where a.add_by = 11728260 -- Erin
   -- where p.part_key = 2585595  --87
   -- where p.part_no like '10103344'
-- where p.part_no like '10103353%'
 --   order by p.part_no,o.Operation_Code,a.assembly_no
-- )s1
