...
*/
 ...
 -- select count(*)
  ...
  where p.pcn = @PCN
  and p.unit_price != 0
--  and i.active = 1  -- 6385 or 15070 altogether
...
insert into #purchasing_item_summary (id,pcn,item_key,tool_key,item_no,trim,tool_type_code,description,unit_price,storage_location,active)
select 
...
inner join 
(
  select i.plexus_customer_no pcn,i.item_key,i.item_no,i.active from purchasing_v_item_e i
  where i.plexus_customer_no = @PCN
--  and i.active = 1  -- need to show obsolete records since tool lists have obsolete tooling in them
  and i.item_no not like '%[A-Z-]%'  --17558
) i
on s.pcn=i.pcn
and s.item_key=i.item_key
...
if (@EndId = -1)
BEGIN
  select @EndId=count(*) from #purchasing_item_summary 
END
...
--where i.item_no like '%8742'
where i.id between @StartId and @EndId
order by i.item_no
-- select * from purchasing_v_item i where i.item_no like '%8743'
/*
