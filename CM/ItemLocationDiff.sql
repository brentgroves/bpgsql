...
into station101920
from STATION
select count(*) cnt from station101920
-- 12679 20/19
-- 12679 10/12
--12679 10/05
--12679 10/01
...
select top 100 * from station101920
-- Upload the item_location table into PlxSupplyItemLocation table.
CREATE TABLE Cribmaster.dbo.PlxSupplyItemLocation101920 (
	item_no varchar(50),
	location varchar(50),
	quantity integer
)
...
Bulk insert PlxSupplyItemLocation101920
from 'c:\il2019GT12500.csv'
with
(
	fieldterminator = ',',
	rowterminator = '\n'
...
count(*)
-- top 1000 * 
from PlxSupplyItemLocation101920 
-- Check for duplicates
select count(*)
from 
(
select distinct item_no,location from PlxSupplyItemLocation101920
)s1
-- 14595 10/19
-- 14590 10/12
-- 14583 10/05
...
--il.item_no,il.location,il.quantity
 into nic101920 --Plex supply items with the default location and a quantity = 0
-- count(*) 
 -- 3782 10/19
 -- 3779 10/12
 -- 3779 10/01
...
where 
 il.quantity <> st.BinQuantity -- 288 10/19,242 10/12,130 10/05, 247 10/01, 267 09/28, 291, 09/21,184 09/14,212 09/08, 581 09/01,158 08/17,261 07/20, 246 07/13, 386 07/01,309 06/30, 179 06/15,06/09 333,06/02 89,06/01 126, --05/26=115,05/18=172,340,334,351,381,421,375,304,409,312, 213,177,1330,1319,510,293,376,416,417,472, 342,353, 455,406,384	
-- il.quantity > st.BinQuantity -- 135 10/19,93 10/12,89 10/05,112 10/01,142 09/21,74 09/14,77 09/08, 211 09/01,48 08/17,50 07/20, 148 07/01,112 06/30,60 06/22,43 06/15,145 06/09,12 06/02,58 06/01 05/26=40,05/18=84,120,143,107,95,154,149,122, 124, 124,77,51, 492,538,134,140,171,172,177,120,138,131,61
-- il.quantity < st.BinQuantity -- 153 10/19,149 10/12, 41 10/05,135 10/01,125 09/28,167 09/21,110 09/14,135 09/11, 370 09/01,110 08/17/20,211 07/20,173 07/13,238 07/01, 197 06/30,190 06/22,136 06/15,188 06/09, 77 06/02 --68 06/01--05/26=75,05/18=88,220,191,244,286,267,226,182,285,188,136,126,838,781,376,168,236,245,245,295,222,311,215, 316,275,105
