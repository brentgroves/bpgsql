...
--select getdate() AT TIME ZONE 'UTC' AT TIME ZONE 'Eastern Standard Time'
exec @R=Kors.notifications_get_op @PCN,@Level=@Lvl,@Destinations = @Dest OUT; --, @cur_time
--exec @R=Kors.notifications_get_op @PCN,@Level=@Lvl,@Destinations = @Dest OUT,@dbg_time=@cur_time; --, @cur_time
select @R,@Dest;

--drop procedure Kors.notifications_get_op;
create procedure Kors.notifications_get_op(
 @PCN integer=295932,
 @Level integer=1,
