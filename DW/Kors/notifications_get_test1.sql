/*
 * 09/07/21 THIS IS THE SAME AS NOTIFICATIONS_GET THE LOGIC IS IDENTICAL
 * YOU CAN USE THIS FOR TESTING OR DELETE IT
 */
...
--select getdate() AT TIME ZONE 'UTC' AT TIME ZONE 'Eastern Standard Time'
exec @R=Kors.notifications_get_rs @PCN,@Level=@Lvl
--exec @R=Kors.notifications_get_rs @PCN,@Level=@Lvl,@Destinations = @Dest OUT,@dbg_time=@cur_time; --, @cur_time
select @R,@Dest;
...
--		from Kors.notification_test1 n
		-- select * from Kors.recipient r2 
		-- select * from Kors.notification n2 
		inner join Kors.recipient r 
		on n.pcn=r.pcn
		and n.customer_employee_no=r.customer_employee_no
