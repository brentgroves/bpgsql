...
--	anchor_member(pcn,account_key,account_no,period,next_period,max_fiscal_period)
...
--	SELECT * FROM   account_period_balance_low order by period OPTION (MAXRECURSION 210);  -- 37,161
--	SELECT count(*) FROM   account_period_balance_low OPTION (MAXRECURSION 210);  -- 37,161
	--where account_no = '41100-000-0000'
calc_ytd_low (pcn,period,next_period,account_no,debit,ytd_debit,credit,ytd_credit,balance,ytd_balance)
AS
(
    -- Anchor member
    select
	...
	inner join max_fiscal_period m 
    on y.pcn=m.pcn
